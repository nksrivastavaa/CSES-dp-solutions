...
void precomputeDP(){
    ...
    /*
    dp[i][0] --> total ways to construct i height tower of width 1 when
                 constructed from i-1 height tower
        <---i--->    |_|_|   |_| |   | |_|   | | |   |_|_|
        <---i-1--->  |_ _|   |_|_|   |_|_|   |_|_|   |_|_|
    dp[i][1] --> total ways to construct i height tower of width 2 when
                 constructed from i-1 height tower
        <---i--->    |_ _|   |   |   |_ _|  
        <---i-1--->  |_ _|   |_ _|   |_|_|   
    */

    dp.assign(N+1, vl(2, 0LL));

    dp[1][0] = dp[1][1] = 1;
    
    for(int i = 2; i<=N; i++){
        //dp[i][0] = 4*dp[i-1][0] + dp[i-1][1]
        dp[i][0] += (((2*dp[i-1][0])%mod + (2*dp[i-1][0])%mod)%mod + dp[i-1][1])%mod;
