...
void solve() {
    ...
    vi lis; /* it doesn't necessarily contain elements in the LIS, but it's size
    will give length of the lis */

    rep(i, n){
        if(i==0) lis.pb(x[i]);
        else{
            if(x[i] > lis[lis.size()-1]){
                lis.pb(x[i]);
            }
            else{
                *lower_bound(all(lis), x[i]) = x[i]; /* Use lower bound cause we need 
                strictly increasing subsequence. Can use upper bound if non-decreasing
