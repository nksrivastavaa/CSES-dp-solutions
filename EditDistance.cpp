...
void solve() {
    ...
    //dp[n][m] --> matching string of length n to string of length m;
    ...
    for(int i = 0; i<=n; i++){
        for(int j = 0; j<=m; j++){
            ...
            if(i==0) dp[i][j] = j; //insert j elements in 1st string

            else if(j==0) dp[i][j] = i; //insert i elements in 2nd string

            else if(a[i-1] == b[j-1]) dp[i][j] = dp[i-1][j-1]; /*last char matching
            so dp[i][j] is equivalent to dp[i-1][j-1] as there is nothing to do here*/
