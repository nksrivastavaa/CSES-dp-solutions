...
void solve() {
    ...
    vvl dp(n+2, vl(n+2, 0LL));

    /*  
    left - l, right - r.

    After p1 chooses an element, p2 will choose an element such that the next element p1 gets 
    is minimum. This is because p2 plays optimally as well. The recurrence will be like : 

    dp[l][r] = max(x[l] + min(dp[l+2][r], dp[l+1][r-1]), x[r] + min(dp[l+1][r-1], dp[l][r-2]));
    (it stores max score p1 can achieve when game is played from array index l to array index r)
    
    dp[0][n-1] will give the ans because the array ranges from index 0 to n-1.
    */
