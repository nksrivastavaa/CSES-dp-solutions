...
void solve() {
    ...
    //notice that both sets have equal sum = n*(n+1)/4
    ...
    vvi dp(n+1, vi(n*(n+1)/4 + 1, 0)); // dp[i][j] = dp[choose any of 1st i numbers][to make sum j]
    ...
    cout<<mod_div(dp[n][n*(n+1)/4], 2, mod)<<nl; //divide by 2 cause we only need the ordered pairs.
}   
